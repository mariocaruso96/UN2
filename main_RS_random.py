# -*- coding: utf-8 -*-
"""
Created on Wed Mar 18 15:44:21 2026
...
def find_candidates_features(
    ...
    # Se alcuni score NON sono nulli:
        ...
        # Trasformo gli scores in DataFrame mantenendo solo le feature ammissibili.
        # Nota:
        # - mode == "Probabilities": probabilità proporzionali agli score SHAP-derived.
        # - mode == "Casual": probabilità uniforme = 1 / numero di feature ammissibili.
        ...
        if mode == "Probabilities":
            ...
            scores["Probability"] = 1.0 / len(scores)
            scores = scores.sort_values(by = "Scores", ascending = False)
        else:
            raise ValueError("mode must be either 'Probabilities' or 'Casual'.")
...
def save_rs_outputs(
    ...
    country = _safe_folder_name(parameters["country"])  + "_random"
...
def get_project_root() -> Path:
    # .../UN_last/Codice/6 Recommender System.py -> parents[1] = .../UN
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument(
        ...
        dest="rs_config",
        default=None,
        help="Path to rs_config.json (absolute or relative)"
    )
...
# default: usa la struttura del progetto su ReCaS (o ovunque)
default_cfg_dir = PROJECT_ROOT / "configs"
rs_cfg_path = Path(args.rs_config) if args.rs_config else (default_cfg_dir / "rs_config.json")
rs_cfg_path = rs_cfg_path.expanduser()

# Carico l'unico config con sezioni: "paths" e "arguments"
rs_config = load_json_config(str(rs_cfg_path))
...
print(f"# features in data_df: {len(data_df.columns)}")
print(f"# features in top_features_df: {len(top_features_df.index)}")
...
run_dir = save_rs_outputs(
    run_base_path=r"/lustrehome/mariocaruso96/UN/RS/Risultati/",
...
print(f"Results for {country} were saved in {run_dir}")
